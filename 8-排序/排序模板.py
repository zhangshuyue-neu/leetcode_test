...
# 最大堆  自己实现
def heapify(tree, n, i):  # tree[i]为根节点的子树进行堆排序   调整堆
    c1 = 2 * i + 1
    c2 = 2 * i + 2
    max_index = i  # 小顶堆这里改下
...
def build_heap(tree, n):   # 从parent->0节点分别进行一次堆排序 就建立了这个树的堆   建堆
    last_node = n - 1
    parent = (last_node - 1) // 2
    for i in range(parent, -1, -1):
        heapify(tree, n, i)
def heap_sort(tree, n):  # 堆排序
    build_heap(tree, n)
...
# print(nums)  # [0, 2, 3, 4, 5, 6]

# import heapq
# nums = [3, 2, 0, 5, 6, 4]
# heapq.heapify(nums)   # 第一种掉包排序实现（建立最小堆）
# print(nums)  # [0, 2, 3, 5, 6, 4]
# hp = []
