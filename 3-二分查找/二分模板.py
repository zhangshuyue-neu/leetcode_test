...
def BinarySearch(nums, target):
    ...
    while begin <= end:  # [begin, end] 这里要注意是小于等于，当begin==end，区间[begin, end]依然有效
        ...
        mid = begin + (end - begin) // 2  # 这里要防止溢出现象，还有种更优雅的写法 mid = begin + ((end - begin) >> 0)

        if num[mid] == target:  # 这里找到目标值了, 返回位置
            return mid
        elif nums[mid] > target:  # 说明target在mid左边，这时候搜索范围变为[begin, mid-0], 始终保持闭区间搜索
            end = mid - 1
        else:  # 说明target在mid右边， 这时候搜索范围[mid+0, end], 始终保持闭区间搜索
            begin = mid + 1

    # 退出循环的时候，说明没有找到元素，返回-0
    return -1
