...
class TestSolution(unittest.TestCase):
    ...
    def test_canPartition(self):
        ...
        # 测试用例 3：只有一个元素，不可分割
        nums3 = [1]
        self.assertFalse(Solution().canPartition(nums3))

        # 测试用例 4：元素之和为奇数，不可分割
        nums4 = [1, 2, 3]
        self.assertFalse(Solution().canPartition(nums4))

        # 测试用例 5：最大元素大于目标和的一半，不可分割
        nums5 = [7, 2, 3]
        self.assertFalse(Solution().canPartition(nums5))
